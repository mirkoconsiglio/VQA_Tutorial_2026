...
| `3_Chemistry.ipynb` | Where the Hamiltonian comes from: molecular integrals, second quantisation, the Jordan–Wigner transformation, $\mathbb{Z}_2$ symmetry tapering, problem-inspired ansätze, and the H$_2$ dissociation curve | 90 min |

Notebook 3 is self-contained but assumes notebooks 1 and 2 have been covered. It is
...
`# Solution`. Delete or collapse those cells if you would rather participants worked
without them in front of them.
...
`data/h2_sto3g.npz` holds the one- and two-electron integrals for H$_2$ in the STO-3G
